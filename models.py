...
class Dis(torch.nn.Module):
    def __init__(self):
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(
                in_channels=3, out_channels=64, kernel_size=4, stride=2, padding=(1, 1)
            ),
        ...
        self.block2 = nn.Sequential(
            ...
            nn.BatchNorm2d(
                128, eps=1e-05, momentum=0.1, affine=True, tracking_running_stats=True
            ),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout(0.25),
        )
        ...
        self.block5 = nn.Sequential(
            nn.Conv2d(
                in_channels=512, out_channels=1, kernel_size=4, stride=1, padding=(1, 1)
            ),
            nn.Flatten(),
...
class CGAN(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.lda = 0.001  # hyperparameter for l1 loss
        self.net_G = Unet().to(self.device)
        self.net_D = Dis().to(self.device)
        self.learning_rate = 0.0005

    def forward(self, imgs_l, imgs_ab):
        self.fake_color = self.net_G(imgs_l)
        fake_prob = self.net_D(self.fake_color, imgs_l)
    ...
    def loss(self, fake_prob, true_prob):
        ...
        return torch.mean(gan_loss) + l1_loss
...
class Unet(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder0 = nn.Sequential(
            nn.Conv2d(
                in_channels=1, out_channels=64, kernel_size=3, stride=1, padding="same"
            ),
            nn.BatchNorm2d(64),
            nn.LeakyReLU(0.2),
            nn.Conv2d(
        ...
        self.up_sample4 = nn.ConvTranspose2d(
        ...
        self.up_sample3 = nn.ConvTranspose2d(
...
class Colorization(nn.Module):
    def __init__(self, depth_after_fusion):
        ...
        self.after_fusion = nn.Conv2d(
            1000 + depth_after_fusion, depth_after_fusion, kernel_size=1
        )
...
def Vgg():
    model = torchvision.models.vgg16(
        weights="DEFAULT"
    )  # must use this in order to run on Department machines.
